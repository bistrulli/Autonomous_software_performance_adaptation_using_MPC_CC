...
addpath("../")
closedUnbalancedQNwithDelaySystemInit
...
optimize([constrO; q_size==400],cost,sdpsettings('solver','gurobi','debug',1))
%%
steadyState_controller=optimizer(constrO,cost,sdpsettings('solver','gurobi'),[q_size],[vec(cores);x_inst]);
steadyState_controllerFronzenVMs=optimizer(constrO,cost,sdpsettings('solver','gurobi'),[q_size;x_inst],[vec(cores)]);
stepPlotInstancesSS
